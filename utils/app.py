...
class VideoFrame(customtkinter.CTkFrame):
    def __init__(self, master, detector, width=1024, height = 720):
...
class DropEntryFrame(customtkinter.CTkFrame):
    def __init__(self, master, detector, source = None):
        ...
        self.dropmenu = DropMenu(self, detector= self.detector, source= self.source)
        ...
        self.entyrmenu = EntryMenu(self, detector= self.detector)
...
class App(customtkinter.CTk):
    def __init__(self, detector, window_title = "Hello World", delay = 1, source = None,) -> None:
        super().__init__()
        self.detector = detector
        self.delay = delay
        self.source= source
        ...
        self.videoframe = VideoFrame(self, detector= self.detector,width= self.width, height=self.height)
        self.videoframe.grid(column=0, row=0, padx = 10, pady= (20,20), sticky= 'nw')

        ## Drop-Down Menu & Source Entry Widget
        self.dropentrymenu = DropEntryFrame(self, detector= self.detector, source= self.source)
